...
const System = {
  ...
  keys: async function () {
    return await fetch(`${API_BASE}/setup-complete`)
      ...
      .catch(() => {
        // Return mock data for testing interface when backend is not available
        console.warn("Backend not available, using mock system keys for testing");
        return {
          MultiUserMode: false,
          RequiresAuth: false,
          LLMProvider: "openai",
  ...
  checkAuth: async function (currentToken = null) {
    const valid = await fetch(`${API_BASE}/system/check-token`, {
      ...
      .then((res) => res.ok)
      .catch(() => {
        // For testing interface, assume auth is valid if we have a token
        console.warn("Backend not available, mocking auth check for testing");
        return !!currentToken || !!localStorage.getItem(AUTH_TOKEN);
      });
  ...
  fetchSupportEmail: async function () {
    ...
    const { supportEmail, error } = await fetch(
      ...
      .then((res) => res.json())
      .catch((e) => {
        console.warn("Backend not available, using default support email for testing");
        return { supportEmail: "", error: null };
      });
  ...
  fetchLogo: async function () {
    ...
    return await fetch(url, {
      ...
      .catch((e) => {
        // For testing interface, return default logo state when backend unavailable
        console.warn("Backend not available, using default logo for testing");
  ...
  fetchPfp: async function (id) {
    return await fetch(`${API_BASE}/system/pfp/${id}`, {
      ...
      .catch((e) => {
        // For testing interface, return null when backend unavailable
        console.warn("Backend not available, no profile picture for testing");
        return null;
      });
  },
